...
def ores_(i_c, params_list):
    res = BasicBlock(i_c,i_c)
    return res

def dres_(i_c, params_list):
    res = DilatieBasicBlock(i_c,i_c)
    return res
...
class DilatieBasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1):
        super(DilatieBasicBlock, self).__init__()
        m = OrderedDict()
        m['conv1'] = nn.Conv2d(inplanes, planes, 3,1,2,2)
        m['bn1'] = nn.BatchNorm2d(planes)
        m['relu1'] = nn.ReLU(inplace=True)
        m['conv2'] = nn.Conv2d(planes, planes, 3,1,2,2)
        m['bn2'] = nn.BatchNorm2d(planes)
        self.group1 = nn.Sequential(m)
        self.relu= nn.Sequential(nn.ReLU(inplace=True))
